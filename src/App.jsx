...
function DesignEditor({ notify }) {
  ...
  const shapeCategories = { Geometry:["square","rectangle","circle","ellipse","triangle","diamond","pentagram","hexagram","heptagram","octagon","heart","arrow"], Contacts:["phone","mail","send","world","like","dislike","check","ok"], Animals:["cat","dog","bird","fish","butterfly","paw"], Objects:["airplane","camera","home","car","key","gift","umbrella","lightning"], Flags:["flag","flag-round","banner"], Emojis:["smile","wink","laugh","sad","love","fire","star"], Fruits:["apple","orange","banana","watermelon","lemon"], Vegetables:["carrot","pepper","broccoli","tomato","corn"] };
  const shapeLibrary = Object.entries(shapeCategories).flatMap(([category,items])=>items.map((shape,index)=>({id:`${category}-${index}`,shape,category})));
  ...
  const shapeGlyph = (shape, color) => { const common={fill:color,stroke:"currentColor",strokeWidth:2,strokeLinejoin:"round"}; if(shape==="circle"||shape==="ellipse") return <ellipse cx="50" cy="50" rx={shape==="ellipse"?42:36} ry={shape==="ellipse"?30:36} {...common}/>; if(shape==="square"||shape==="rectangle") return <rect x="12" y="20" width={shape==="rectangle"?76:60} height={shape==="rectangle"?60:60} rx="4" {...common}/>; if(shape==="triangle") return <path d="M50 10 90 88H10Z" {...common}/>; if(shape==="heart"||shape==="love") return <path d="M50 88 14 52C-8 28 24 4 50 30 76 4 108 28 86 52Z" {...common}/>; if(shape.includes("star")||shape.includes("gram")) return <path d="M50 8 61 38 94 38 68 57 78 90 50 70 22 90 32 57 6 38 39 38Z" {...common}/>; if(shape==="check"||shape==="ok") return <path d="m15 52 22 22 48-50" fill="none" stroke={color} strokeWidth="12" strokeLinecap="round"/>; if(shape==="arrow"||shape==="send") return <path d="M12 50h62M52 25l26 25-26 25" fill="none" stroke={color} strokeWidth="10" strokeLinecap="round" strokeLinejoin="round"/>; return <circle cx="50" cy="50" r="34" {...common}/>; };
  const panelContent = () => {
    ...
    if (activeTool === "Elements") return <div className="pro-panel-content"><h3>Elements</h3><p>Vector shapes organized by category.</p>{Object.entries(shapeCategories).map(([category])=><details key={category} open={category==="Geometry"}><summary>{category}<span>{shapeCategories[category].length}</span></summary><div className="asset-library shape-library">{shapeLibrary.filter((item)=>item.category===category).map((item)=><button key={item.id} onClick={()=>addShape(item)} title={item.shape}><svg viewBox="0 0 100 100" aria-hidden="true">{shapeGlyph(item.shape,drawSettings.color)}</svg><span>{item.shape}</span></button>)}</div></details>)}</div>;
        ...
        <aside className="pro-rightpanel">
          ...
          {selected && (
            <div className="layer-properties">
              ...
              {selected.type === "background" && <div className="background-properties"><p>Choose the artboard color.</p><input className="color-field" type="color" value={background} onChange={(e)=>{setBackground(e.target.value);record("Changed background")}}/><div className="swatches">{["#ffffff","#111111","#f5df4d","#6c51f4","#ff694f","#84d6c3","#e7c7a1","#c7d7ff"].map((c)=><button key={c} style={{background:c}} title={c} onClick={()=>{setBackground(c);record("Changed background")}}/> )}</div></div>}
